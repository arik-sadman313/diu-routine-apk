  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800/80 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-slate-100 dark:border-slate-800/60">
        ...
        {action && onAction && (
          <button onClick={onAction} className="text-[11px] font-bold text-accent-600 dark:text-accent-400 hover:text-accent-700 dark:hover:text-accent-300 transition-colors flex items-center gap-0.5">
    ...
    return (
      <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-100 dark:divide-slate-800/60 w-full">
        <div className="p-4 flex-shrink-0 flex items-center justify-between md:flex-col md:items-start md:justify-center md:w-48 gap-2 relative">
          <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-accent-500/5 to-transparent rounded-bl-full pointer-events-none" />
          <div>
            <button 
              onClick={() => setIsOpen(true)}
              className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-accent-500 dark:text-slate-400 dark:hover:text-accent-400 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500/50 rounded p-0.5 -ml-0.5"
            >
          ...
          <div className="flex flex-col items-end md:items-start gap-1">
            <Icon className="w-7 h-7 text-accent-500 dark:text-accent-400 drop-shadow-sm mb-1 hidden md:block" />
        ...
        <>
          ...
          >
            ...
            <div className="max-h-48 overflow-y-auto p-1.5">
              {searching ? (
                <div className="p-4 flex justify-center"><Loader2 className="w-4 h-4 animate-spin text-accent-500" /></div>
              ) : searchError ? (
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-4 md:p-5 flex flex-col sm:flex-row items-center gap-6">
      ...
      <div className="flex-1 w-full flex flex-col justify-center">
        ...
        <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner mb-4 sm:mb-0">
          <div className="h-full bg-gradient-to-r from-accent-500 to-indigo-500 rounded-full transition-all duration-1000 ease-linear" style={{ width: `${timeProgress}%` }} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between sm:justify-center w-full sm:w-auto gap-4 text-center">
        ...
        <div className="flex-1 sm:flex-none">
          <div className="text-xl font-black text-accent-600 dark:text-accent-400">{ongoing}</div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-accent-400/80">Now</div>
        </div>
        <div className="w-px h-6 bg-slate-200 dark:bg-slate-800" />
        <div className="flex-1 sm:flex-none">
        ...
        <div
          className={`px-5 py-5 md:px-6 rounded-3xl shadow-md relative overflow-hidden group text-white cursor-pointer ${
            currentClass
              ...
              : 'bg-gradient-to-br from-accent-600 to-violet-700 shadow-accent-500/20'
        ...
        >
          ...
          {focusClass ? (
            <div className="relative z-10">
              ...
              {getCourseName(focusClass.course_code) && classDetailMode === 'detailed' && (
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold">
                  <div className="bg-black/20 backdrop-blur-md px-2.5 py-1.5 rounded-lg border border-white/10 flex items-center gap-1.5">
                    ...
                    {nextClassDay !== 'Today' ? `${nextClassDay} · ` : ''}{formatRoutineTime(focusClass.start_time, timeFormat)} – {formatRoutineTime(focusClass.end_time, timeFormat)}
                  </div>
                  {showRoom && (
                  ...
                  {showGroup && focusClass.group_code && (
              ...
              {todayClasses.map((c, i) => {
                ...
                return (
                  <div key={i} onClick={() => setSelectedClass(c)} className="relative group cursor-pointer">
                    <div className="absolute -left-[20px] top-3.5 w-3 border-t-2 border-slate-200 dark:border-slate-800 transition-colors group-hover:border-accent-400" />
                    <div className={`absolute -left-[24px] top-2.5 w-2.5 h-2.5 rounded-full border-2 bg-white dark:bg-slate-900 transition-colors ${
                      isCur ? 'border-accent-500 scale-125 shadow-[0_0_8px_rgba(168,85,247,0.5)]' 
                      : isPast ? 'border-slate-300 dark:border-slate-600'
                      : isNext ? 'border-accent-400 dark:border-accent-500 scale-110'
                    ...
                    <div className={`pl-2 pr-2 py-2 rounded-xl transition-all flex flex-col md:flex-row md:items-center justify-between gap-1 md:gap-4 ${
                      ...
                      <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2 md:gap-0 mt-1 md:mt-0 flex-shrink-0">
                        <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap">{formatRoutineTime(c.start_time, timeFormat)} – {formatRoutineTime(c.end_time, timeFormat)}</div>
