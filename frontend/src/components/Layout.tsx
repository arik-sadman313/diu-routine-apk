...
export function Layout() {
  ...
  const { versions, selectedVersion, setSelectedVersionId, refreshOptions } = useAppContext();

  const handleDeleteRoutine = async () => {
    if (!selectedVersion) return;
    if (!window.confirm(`Delete "${selectedVersion.name || 'this routine'}"?\n\nThis will permanently remove the routine and all its associated classes and personal overrides.`)) {
      return;
    }
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                <select
                  ...
                  disabled={deleting}
                  className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-lg px-3 py-2 text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-purple-500/50 outline-none appearance-none font-medium pr-7 truncate cursor-pointer disabled:opacity-50"
                >
                  {versions.map(v => (
            ...
            <div className="flex items-center gap-1.5">
              <div className="relative">
                <select
                  ...
                  onChange={(e) => setSelectedVersionId(Number(e.target.value))}
                  disabled={deleting}
                  className="bg-slate-100 dark:bg-slate-800 border-none rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-purple-500/50 outline-none max-w-[140px] truncate appearance-none pr-6 cursor-pointer disabled:opacity-50"
                >
                  {versions.map(v => (
